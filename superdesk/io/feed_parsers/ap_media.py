    ...
    async def parse(self, s_json, provider=None):
        ...
        if in_item.get("datelinelocation"):
            ...
            located = [
                ...
                and c["country"] == in_item.get("datelinelocation").get("countryname")
            ]
            if len(located) > 1:
                located = [
                    c
                    ...
                    if c["city"] == in_item.get("datelinelocation").get("city")
                    and c["country"] == in_item.get("datelinelocation").get("countryname")
                    and c["state"] == in_item.get("datelinelocation").get("countryareaname")
                ]
            if len(located) == 1:
                item["dateline"] = dict()
                ...
                item["dateline"]["text"] = format_dateline_to_locmmmddsrc(
                    item["dateline"]["located"], get_date(item["firstcreated"]), provider.get("source")
                )
