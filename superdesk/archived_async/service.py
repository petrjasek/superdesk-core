...
from superdesk.types.base import ContentStates, GroupRefRefs, PubStatusType
from superdesk.types.archived import ArchivedResourceModel, QueueState
...
class ArchivedService(AsyncResourceService[ArchivedResourceModel]):
    ...
    async def command_delete(self, lookup: dict[str, Any]) -> None:
        await super().delete_many(lookup)
    ...
    async def find_one(
        ...
        **lookup,
        ...
        if user and item and str(item.task.stage) in await UsersAsyncService().get_invisible_stages_ids(user["_id"]):
    ...
    async def _remove_and_set_kill_properties(
        ...
        article.pubstatus = PubStatusType.CANCELED
        article.updated = utcnow()

        user = get_user()
        article.version_creator = ObjectId(user["_id"])
    ...
    def _get_take_package_id(self, item: ArchivedResourceModel) -> str | None:
        ...
        takes_package = [
            package.package for package in item.linked_in_packages or [] if package.package_type == TAKES_PACKAGE
        ]
    ...
    def _get_package_refs(self, package: ArchivedResourceModel) -> list[GroupRefRefs]:
        """Get refs from the takes package."""

        refs = []
        if package and package.groups:
            refs = next((group.refs for group in package.groups if group.id == MAIN_GROUP), [])
        return refs
