...
class Newsml12FormatterTest(TestCase):
    ...
    package = {
        ...
        "_current_version": 1,
        "groups": [
            {"role": "grpRole:NEP", "id": "root", "refs": [{"idRef": "main"}]},
            {
                "role": "grpRole:main",
                "id": "main",
                "refs": [
    ...
    async def asyncSetUp(self):
        ...
        self._setup_dates(
            [
                ...
                self.picture,
                self.package,
                self.picture_package,
                self.preformatted,
                self.picture_text_package,
    ...
    async def test_format_identification(self):
        ...
        self.assertEqual(
            self.newsml.find("Identification/NewsIdentifier/PublicIdentifier").text, "urn:localhost.abc:5N"
        )
    ...
    async def test_format_news_component(self):
        ...
        self.assertEqual(
            self.newsml.find("NewsComponent/NewsComponent/NewsLines/HeadLine").text,
            "This is a test headline - для теста!",
        )
        self.assertEqual(self.newsml.find("NewsComponent/NewsComponent/NewsLines/ByLine").text, "joe")
        self.assertEqual(self.newsml.find("NewsComponent/NewsComponent/NewsLines/DateLine").text, "sample dateline")
        self.assertEqual(self.newsml.find("NewsComponent/NewsComponent/NewsLines/CreditLine").text, "AAP")
        self.assertEqual(self.newsml.find("NewsComponent/NewsComponent/NewsLines/KeywordLine").text, "slugline")
        self.assertEqual(
            self.newsml.findall("NewsComponent/NewsComponent/DescriptiveMetadata/SubjectCode/Subject")[0].get(
                "FormalName"
            ),
            "02011001",
        )
        self.assertEqual(
            self.newsml.findall("NewsComponent/NewsComponent/DescriptiveMetadata/SubjectCode/Subject")[1].get(
                "FormalName"
            ),
            "02011002",
        )
        self.assertEqual(self.newsml.find("NewsComponent/NewsComponent/DescriptiveMetadata/Property").get("Value"), "a")
        self.assertEqual(
            self.newsml.findall("NewsComponent/NewsComponent/NewsComponent/ContentItem/DataContent")[0].text,
            "sample abstract",
        )
        self.assertEqual(
            self.newsml.findall(
                "NewsComponent/NewsComponent/NewsComponent/ContentItem/DataContent/nitf/body/body.content/p"
            )[0].text,
    ...
    async def test_format_place(self):
        ...
        self.assertEqual(
            self.newsml.find('Location/Property[@FormalName="CountryArea"]').get("Value"), "New South Wales"
        )
    ...
    async def test_format_picture(self):
        ...
        self.assertEqual(
            xml.find("NewsItem/NewsComponent/NewsComponent/NewsLines/HeadLine").text, "NUS CHRISTOPHER PYNE PROTEST"
        )
        ...
        for rendition, value in doc.get("renditions").items():
            ...
            self.assertEqual(
                content_item.find('Characteristics/Property[@FormalName="Width"]').get("Value"), str(value.get("width"))
            )
            self.assertEqual(
                content_item.find('Characteristics/Property[@FormalName="Height"]').get("Value"),
    ...
    async def test_format_video(self):
        ...
        self.assertEqual(
            xml.find("NewsItem/NewsComponent/NewsComponent/NewsLines/KeywordLine").text, "test video keyword"
        )
        for rendition, value in doc.get("renditions").items():
            xpath = './/Role[@FormalName="{}"]/../ContentItem'.format(rendition)
            content_item = xml.find(xpath)
            ...
            self.assertEqual(content_item.find("Format").get("FormalName"), value.get("mimetype"))
            self.assertEqual(
                content_item.find('Characteristics/Property[@FormalName="Width"]').get("Value"),
    ...
    async def test_format_package(self):
        ...
        self.assertEqual(
            xml.find('.//Role[@FormalName="root"]/../NewsComponent/NewsItemRef').get("NewsItem"),
    ...
    async def test_format_picture_text_package(self):
        ...
        self.assertEqual(
            news_item_refs[0].get("NewsItem"), "tag:localhost:2015:5838657b-b3ec-4e5a-9b39-36039e16400b:4N"
        )
        self.assertEqual(
            news_item_refs[1].get("NewsItem"), "tag:localhost:2015:0c12aa0a-82ef-4c58-a363-c5bd8a368037:4N"
        )
