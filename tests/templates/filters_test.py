...
class ConvertDatetimeFiltersTest(TestCase):
    async def test_convert_datetime_utc_no_format(self):
    ...
    async def test_convert_datetime_local_time_no_format(self):
        template_string = '{{ item.versioncreated | format_datetime("Australia/Sydney")}}'
    ...
    async def test_convert_datetime_invalid_date(self):
        template_string = '{{ item.versioncreated | format_datetime("Australia/Sydney", "%Y-%m-%d")}}'
        item = {"versioncreated": "test string"}
        result = await render_template_string(template_string, item=item)
        self.assertEqual(result, "")

    async def test_convert_datetime_invalid_timezone(self):
        template_string = '{{ item.versioncreated | format_datetime("australia/sydney", "%Y-%m-%d")}}'
        item = {"versioncreated": "test string"}
        result = await render_template_string(template_string, item=item)
        self.assertEqual(result, "")

    async def test_convert_datetime_utc_default_timezone(self):
        template_string = "{{ item.versioncreated | format_datetime()}}"
        item = {"versioncreated": "2015-01-01T22:54:53+0000"}
        result = await render_template_string(template_string, item=item)
        self.assertEqual(result, "2015-01-01 23:54:53+01:00")

    async def test_convert_datetime_local_time_default_timezone(self):
        template_string = "{{ item.versioncreated | format_datetime()}}"
        item = {"versioncreated": "2015-01-01T22:54:53+05:30"}
        result = await render_template_string(template_string, item=item)
        self.assertEqual(result, "2015-01-01 18:24:53+01:00")

    async def test_convert_datetime_utc_timezone_format(self):
    ...
    async def test_get_first_paragraph(self):
    ...
    async def test_get_first_paragraph_doesnt_fail_with_empty_body(self):
        template_string = "{{ item.body_html | first_paragraph()}}"
        item = {"headline": "Sample headline"}
        result = await render_template_string(template_string, item=item)
        self.assertEqual(result, "")

    async def test_add_timedelta(self):
