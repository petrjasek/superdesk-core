...
class CheckForUnknownParamsMethodTestCase(ItemsServiceTestCase):
    ...
    async def test_does_not_raise_an_error_on_valid_parameters(self):
        request = MagicMock()
    ...
    async def test_raises_correct_error_on_unexpected_parameters(self):
        request = MagicMock()
    ...
    async def test_raises_descriptive_error_on_filtering_disabled(self):
        request = MagicMock()
        ...
        with self.assertRaises(UnexpectedParameterError) as context:
            instance._check_for_unknown_params(request, whitelist=(), allow_filtering=False)

        ex = context.exception
        self.assertEqual(ex.payload, "Unexpected parameter (q)")

    async def test_raises_descriptive_error_on_disabled_start_date_filtering(self):
        request = MagicMock()
    ...
    async def test_raises_correct_error_on_duplicate_parameters(self):
        ...
        ex = context.exception
        self.assertEqual(ex.payload, "Multiple values received for parameter (foo)")
...
class GetMethodTestCase(ItemsServiceTestCase):
    ...
    async def test_correctly_invokes_parameter_validation(self, fake_check_unknown):
    ...
    async def test_sets_fields_filter_on_request_object(self, fake_set_fields_filter):
        fake_request = MagicMock()
        fake_request.args = MultiDict()
    ...
    async def test_provides_request_object_to_superclass_if_not_given(self):
        lookup = {}
    ...
    async def test_raises_correct_error_on_invalid_start_date_parameter(self):
        request = MagicMock()
    ...
    async def test_raises_correct_error_if_start_date_greater_than_end_date(self):
        request = MagicMock()
        request.args = MultiDict([("start_date", "2015-02-17"), ("end_date", "2015-02-16")])
        lookup = {}
        ...
        ex = context.exception
        self.assertEqual(ex.payload, "Start date must not be greater than end date")

    async def test_allows_start_and_end_dates_to_be_equal(self):
        request = MagicMock()
        request.args = MultiDict([("start_date", "2010-01-28"), ("end_date", "2010-01-28")])
        lookup = {}
        instance = self._make_one()
    ...
    async def test_includes_given_date_range_into_query_filter_if_given(self):
        request = MagicMock()
        request.args = MultiDict([("start_date", "2012-08-21"), ("end_date", "2012-08-26")])
        lookup = {}
    ...
    async def test_sets_end_date_to_today_if_not_given(self, fake_utcnow):
        request = MagicMock()
        ...
        filters = json.loads(args[0].args["filter"])["bool"]["must"]
        date_filter = filters[0].get("range", {}).get("versioncreated", {})
        expected_filter = {"gte": "2012-08-21", "lte": "2014-07-15"}
        self.assertEqual(date_filter, expected_filter)

    async def test_sets_start_date_equal_to_end_date_if_not_given(self):
        request = MagicMock()
        ...
        filters = json.loads(args[0].args["filter"])["bool"]["must"]
        date_filter = filters[0].get("range", {}).get("versioncreated", {})
        expected_filter = {"gte": "2012-08-14", "lte": "2012-08-21"}
        self.assertEqual(date_filter, expected_filter)

    @mock.patch("content_api.items.service.utcnow")
    async def test_sets_end_date_and_start_date_to_today_if_both_not_given(self, fake_utcnow):
        request = MagicMock()
        ...
        filters = json.loads(args[0].args["filter"])["bool"]["must"]
        date_filter = filters[0].get("range", {}).get("versioncreated", {})
        expected_filter = {"gte": "2014-07-08", "lte": "2014-07-15"}
        self.assertEqual(date_filter, expected_filter)

    async def test_creates_correct_query_if_start_and_end_date_are_the_same(self):
        request = MagicMock()
        request.args = MultiDict([("start_date", "2010-09-17"), ("end_date", "2010-09-17")])
        lookup = {}
        ...
        filters = json.loads(args[0].args["filter"])["bool"]["must"]
        date_filter = filters[0].get("range", {}).get("versioncreated", {})
        expected_filter = {"gte": "2010-09-17", "lte": "2010-09-17"}
        self.assertEqual(date_filter, expected_filter)

    @mock.patch("content_api.items.service.utcnow")
    async def test_raises_correct_error_for_start_date_in_future(self, fake_utcnow):
        request = MagicMock()
    ...
    @mock.patch("content_api.items.service.utcnow")
    async def test_raises_correct_error_for_end_date_in_future(self, fake_utcnow):
        request = MagicMock()
    ...
    async def test_raises_error_for_invalid_parameter_for_service(self):
        ...
        ex = context.exception
        self.assertEqual(ex.payload, "Bad parameter value for Parameter ({})".format("service"))

    async def test_set_filter_for_service(self):
        request = MagicMock()
    ...
    async def test_raises_error_for_invalid_parameter_for_urgency(self):
        request = MagicMock()
        ...
        ex = context.exception
        self.assertEqual(ex.payload, "Bad parameter value for Parameter ({})".format("urgency"))

    async def test_set_filter_for_urgency(self):
        request = MagicMock()
    ...
    async def test_set_filter_for_q(self):
...
class ParseIsoDateMethodTestCase(ItemsServiceTestCase):
    ...
    async def test_returns_none_if_none_given(self):
        klass = self._get_target_class()
        result = klass._parse_iso_date(None)
        self.assertIsNone(result)

    async def test_returns_date_object_on_valid_iso_date_string(self):
        klass = self._get_target_class()
        result = klass._parse_iso_date("2015-05-15")
        self.assertEqual(result, date(2015, 5, 15))

    async def test_raises_value_error_on_invalid_iso_date_string(self):
        klass = self._get_target_class()
...
class SetFieldsFilterMethodTestCase(ItemsServiceTestCase):
    ...
    async def test_raises_error_if_requesting_to_exclude_required_field(self):
        request = MagicMock()
    ...
    async def test_raises_error_if_field_whitelist_and_blacklist_both_given(self):
        ...
        ex = context.exception
        self.assertEqual(ex.payload, "Cannot both include and exclude content fields at the same time.")

    async def test_raises_error_if_whitelisting_unknown_content_field(self):
        request = MagicMock()
        ...
        with mock.patch.object(ItemsResource, "schema", new=fake_schema):  # @UndefinedVariable
            ...
            ex = context.exception
            self.assertEqual(ex.payload, "Unknown content field to include (field_x).")

    async def test_raises_error_if_blacklisting_unknown_content_field(self):
        request = MagicMock()
        ...
        with mock.patch.object(ItemsResource, "schema", new=fake_schema):  # @UndefinedVariable
            ...
            ex = context.exception
            self.assertEqual(ex.payload, "Unknown content field to exclude (field_x).")

    async def test_filters_out_blacklisted_fields_if_requested(self):
        request = MagicMock()
    ...
    async def test_filters_out_all_but_whitelisted_fields_if_requested(self):
        request = MagicMock()
...
class FindOneMethodTestCase(ItemsServiceTestCase):
    ...
    async def test_invokes_superclass_method_with_given_arguments(self):
        request = MagicMock()
        request.args = MultiDict()
        lookup = {"_id": "my_item"}
...
class OnFetchedItemMethodTestCase(ItemsServiceTestCase):
    ...
    async def test_does_not_remove_hateoas_links_from_fetched_document(self):
        document = {
            "_id": "item:123",
            "headline": "breaking news",
            "_links": {"self": {"href": "link/to/item/itself", "title": "Item"}},
        }

        instance = self._make_one(datasource="items")
        instance.on_fetched_item(document)

        expected_links = {"self": {"href": "link/to/item/itself", "title": "Item"}}
...
class OnFetchedMethodTestCase(ItemsServiceTestCase):
    ...
    async def test_does_not_remove_hateoas_links_from_fetched_documents(self):
        result = {
            "_items": [
                {
                    "_id": "item:123",
                    ...
                    "_updated": "12345abcde",
                    "headline": "breaking news",
                    "_links": {"self": {"href": "link/to/item_123", "title": "Item"}},
                },
                {
                    "_id": "item:555",
                    "_etag": "67890fedcb",
                    ...
                    "_updated": "2121abab",
                    "headline": "good news",
                    "_links": {"self": {"href": "link/to/item_555", "title": "Item"}},
                },
            ]
        }
        ...
        expected_links = {"self": {"href": "link/to/item_123", "title": "Item"}}
        self.assertEqual(documents[0].get("_links"), expected_links)

        expected_links = {"self": {"href": "link/to/item_555", "title": "Item"}}
        self.assertEqual(documents[1].get("_links"), expected_links)

    async def test_sets_collection_self_link_to_relative_original_url(self):
        result = {"_items": [], "_links": {"self": {"href": "foo/bar/baz"}}}

        request_url = "items?start_date=1975-12-31#foo"
        async with self.app.test_request_context(request_url):
            instance = self._make_one(datasource="items")
            instance.on_fetched(result)
    ...
    async def test_removes_associated_item_if_subscriber_is_not_entitled(self, fake_g):
            ...
            "_items": [
                {
                    ...
                    "associations": {"featuremedia": {"_id": "a1", "subscribers": ["test"]}},
                },
                {
                    "_id": "item:555",
                    "headline": "another item",
                    "associations": {"featuremedia": {"_id": "a2", "subscribers": ["test2"]}},
                },
            ]
        }

        async with self.app.test_request_context("items"):
            instance = self._make_one(datasource="items")
            instance.on_fetched(result)
...
class GetUriMethodTestCase(ItemsServiceTestCase):
    ...
    async def test_generates_correct_uri_for_composite_items(self):
